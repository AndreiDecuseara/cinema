...
namespace Updivision\Datatable\Livewire\Traits;
 ...
 *
 * @package Updivision\Datatable\Livewire\Traits
 */
trait SyncWithDatatable
{
    /**
     * $syncItem = [ * | search | filters | sort | pagination | queryString | queryStringSearch | queryStringFilters | queryStringSort | queryStringPagination | cacheDataTable | getCachedDataTable ]
     *
    ...
    {
        if (in_array($syncItem, ['*', 'search']) && $this->datatable->isSearchable()) {
            $this->datatable->setSearch($this->search);
        }

        if (in_array($syncItem, ['*', 'filters']) && $this->datatable->isFilterable()) {
            $this->filters = $this->datatable->filters->setValues($this->filters)->toArray();
        }
        ...
        if (in_array($syncItem, ['queryString', 'queryStringSearch', 'queryStringFilters', 'queryStringSort', 'queryStringPagination'])) {
            if (!$this->datatable->hasQueryStrings) {
                ...
                if (in_array($syncItem, ['queryString', 'queryStringSearch']) && $this->datatable->isSearchable()) {
                    $this->queryString = collect($this->queryString)->merge(['search' => ['as' => "{$this->datatable->name}_search", 'except' => '']])->toArray();
