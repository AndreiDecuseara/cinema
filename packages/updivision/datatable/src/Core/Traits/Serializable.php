...
trait Serializable
{
    ...
    public function __serialize(): array
    {
        ...
        foreach ($props as $prop) {
            if ($prop->getType()->getName() === 'Closure' && isset($this->{$prop->getName()})) {
                $property = 'Closure_' . $prop->getName();
                $data = serialize(new SerializableClosure($this->{$prop->getName()}));
            } elseif (Str::contains($prop->getType()->getName(), ['ColumnsCollection', 'ActionsCollection', 'FiltersCollection'])) {
    ...
    public function __unserialize(array $data): void
    {
        ...
        foreach ($props as $prop) {
            if ($prop->getType()->getName() === 'Closure' && isset($data['Closure_' . $prop->getName()])) {
                $this->{$prop->getName()} = unserialize($data['Closure_' . $prop->getName()])->getClosure();
            } elseif (Str::contains($prop->getType()->getName(), ['ColumnsCollection', 'ActionsCollection', 'FiltersCollection'])) {
                $this->{$prop->getName()} = $prop->getType()->getName()::make($data[$prop->getName()]);
